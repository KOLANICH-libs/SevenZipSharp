		...
		IDisposable
	{
		...
		private bool EventsForGetStream(uint index){
			if (!FastCompression) {
				...
				var fiea = new FileNameEventArgs(_files.Length != 0 ? _files[index].Name : _entries[index],
			...
			try {
				switch (propID) {
					...
					case ItemPropId.CreationTime:
						...
						if (_updateData.Mode != InternalCompressionMode.Modify)
							value.Int64Value = _files.Length == 0
