...
namespace SevenZip
{
	...
	internal static class SevenZipLibraryManager
	{
		...
		/// </remarks>
		private static string _libraryFileName = find7zLibrary();
#endif
#if WINCE 		
		...
		public static IInArchive InArchive(InArchiveFormat format, object user){
			lock (_syncRoot) {
				if (_inArchives[user][format] != null) return _inArchives[user][format];
#if !WINCE && !MONO
				var sp = new SecurityPermission(SecurityPermissionFlag.UnmanagedCode);
				sp.Demand();
		...
		private struct ArchiverDescription
		{
			public string regPath, regKey, pathAddition;

			public ArchiverDescription(string regPath, string regKey, string pathAddition){
		...
		private static string find7zLibrary(){
			ArchiverDescription[] archivers ={
				new ArchiverDescription(@"HKEY_LOCAL_MACHINE\SOFTWARE\7-Zip", "Path32", ""),
				new ArchiverDescription(@"HKEY_LOCAL_MACHINE\SOFTWARE\WinRAR", "exe32", "\\Formats\\")
			};
			string[] archiverLibraryNames = {"7z", "7za", "7zxa"};

			foreach (var libName in archiverLibraryNames) {
				var path = libName + ".dll";
				if (File.Exists(path)) return path;
			}
			byte bestLibraryI = byte.MaxValue;
			string bestPath = null;
			for (byte j = 0; j < archivers.Length; j++) {
				object aPathBush = Microsoft.Win32.Registry.GetValue(archivers[j].regPath, archivers[j].regKey, null);
				if (aPathBush == null) continue;
				string aPath = Path.GetDirectoryName(aPathBush.ToString());
				if (!Directory.Exists(aPath)) continue;
				for (byte i = 0; i < archiverLibraryNames.Length; i++) {
					var path = aPath + archivers[j].pathAddition + archiverLibraryNames[i] + ".dll";
					if (File.Exists(path) && i < bestLibraryI) {
						if (i == 0)
							return path;
						bestLibraryI = i;
						bestPath = path;
