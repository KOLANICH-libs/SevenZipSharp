...
#endif
	{
		...
		private IOutArchive MakeOutArchive(IInStream inArchiveStream){
			...
			using (ArchiveOpenCallback openCallback = GetArchiveOpenCallback()) {
				ulong checkPos = 1 << 15;
				OperationResult res = inArchive.Open(inArchiveStream, ref checkPos, openCallback);
				if (res != OperationResult.Ok)
					if (
						!ThrowException(null, new SevenZipArchiveException("Can not update the archive: Open() failed : " + res)))
						return null;
			...
			foreach (var pair in fileDictionary)
				if (pair.Value == null)
					...
					if (!File.Exists(pair.Value))
						throw new CompressionFailedException("The file corresponding to the archive entry \"" + pair.Key +
															 "\" does not exist.");
					streamDict.Add(
						pair.Key,
