...
namespace SevenZip
{
...
#endif
	{
		...
		private OperationResult OpenArchiveInner(IInStream archiveStream,
			...
			ulong checkPos = 1 << 15;
			OperationResult res = _archive.Open(archiveStream, ref checkPos, openCallback);
			return res;
		}
		...
		private bool OpenArchive(IInStream archiveStream,
			...
			if (!_opened) {
				OperationResult res = OpenArchiveInner(archiveStream, openCallback);
				if (res != OperationResult.Ok)
					if (!ThrowException(null, new SevenZipArchiveException(res.ToString())))
						return false;
		...
		private ArchiveExtractCallback GetArchiveExtractCallback(string directory, int filesCount,
			...
			var aec = String.IsNullOrEmpty(Password)
				...
				: new ArchiveExtractCallback(_archive, directory, filesCount, PreserveDirectoryStructure, actualIndexes, Password,
					this);
		...
		public void ExtractFile(string fileName, Stream stream){
			...
			foreach (ArchiveFileInfo afi in _archiveFileData) {
				if (afi.FileName != fileName || afi.IsDirectory) continue;
				index = afi.Index;
				break;
			}
			if (index == -1) {
